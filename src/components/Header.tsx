...
export default function Header({ role, fullName, avatarUrl, onQuickAction, theme, toggleTheme }: HeaderProps) {
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2.5 w-80 bg-[#101726] border border-slate-800 rounded-2xl shadow-2xl p-4 z-50 text-slate-200">
              <div className="flex justify-between items-center pb-2 border-b border-slate-800 mb-2">
                <span className="text-xs font-bold text-white uppercase tracking-widest text-slate-350">Alert Notification Hub</span>
                <span className="text-[10px] text-blue-400 hover:underline cursor-pointer">Mark all read</span>
              </div>
              <div className="space-y-3 max-h-60 overflow-y-auto">
                {mockNotifications.map((notif) => (
                  <div key={notif.id} className="text-xs hover:bg-slate-900/30 p-1.5 rounded-lg transition-colors">
                    <div className="flex justify-between font-bold text-slate-100">
                      <span className="truncate pr-1">{notif.title}</span>
                      {notif.unread && <span className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1.5 flex-none"></span>}
                    </div>
                    <p className="text-slate-400 text-[11px] leading-snug">{notif.desc}</p>
